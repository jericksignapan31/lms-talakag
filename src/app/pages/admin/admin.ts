...
@Component({
  ...
  template: `
              ...
              <tr *ngFor="let admin of admins()" class="border-b border-gray-200 hover:bg-gray-50 transition">
                ...
                <td class="px-4 py-3">
                  <span class="px-3 py-1 rounded-full text-xs font-semibold"
                        [ngClass]="admin.role === 'super-admin' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'">
                    {{ admin.role }}
                  </span>
                </td>
                <td class="px-4 py-3">
                  <span class="px-3 py-1 rounded-full text-xs font-semibold"
                        [ngClass]="admin.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'">
                    {{ admin.status }}
                  </span>
                </td>
                <td class="px-4 py-3 text-gray-600">{{ admin.lastLogin ? (admin.lastLogin | date: 'short') : 'Never' }}</td>
                <td class="px-4 py-3 text-center">
                  <button pButton pRipple icon="pi pi-pencil" class="p-button-rounded p-button-warning p-button-sm mr-2" 
                          (click)="editAdmin(admin)"></button>
                  <button pButton pRipple icon="pi pi-trash" class="p-button-rounded p-button-danger p-button-sm" 
                          (click)="confirmDelete(admin)"></button>
                </td>
              </tr>
    ...
    <!-- Add/Edit Dialog -->
    <p-dialog [visible]="displayDialog()" (visibleChange)="displayDialog.set($event)" [header]="editingAdmin() ? 'Edit Admin' : 'Add Admin'" 
      ...
      <div class="space-y-4">
        ...
        <div>
          ...
          <p-select [(ngModel)]="adminForm.role" [options]="roleOptions" 
                    optionLabel="label" optionValue="value" class="w-full"></p-select>
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">Status</label>
          <p-select [(ngModel)]="adminForm.status" [options]="statusOptions" 
                    optionLabel="label" optionValue="value" class="w-full"></p-select>
        </div>
...
export class AdminComponent implements OnInit, OnDestroy {
  ...
  saveAdmin() {
    ...
    if (this.editingAdmin()) {
      ...
      this.adminService.updateAdmin(this.editingAdmin()!.id!, adminToUpdate).then(() => {
      ...
      this.adminService.createAdminAccountAndRecord(newAdmin, this.adminForm.password).then(() => {
  ...
  deleteAdmin(admin: Admin) {
    this.adminService.deleteAdminAccountAndRecord(admin.id!, admin.email).then(() => {
