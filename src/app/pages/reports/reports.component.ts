...
import { ReportsService, BorrowingHistoryReport, BookPopularityReport, StudentPerformanceReport, BorrowerListReport } from '../../services/reports.service';

@Component({
    selector: 'app-reports',
    standalone: true,
    imports: [CommonModule, FormsModule, Button, Card, TableModule, Tabs, TabPanel, TabPanels, TabList, Tab, Select, DatePicker, ChartModule, Tag],
    template: `
            ...
            <p-tabs value="0">
                ...
                <p-tabpanels>
                            ...
                            <div class="filters-section">
                                ...
                                <div class="filter-grid">
                                    <div class="filter-item">
                                        ...
                                        <p-datePicker [(ngModel)]="dateRange" selectionMode="range" [showIcon]="true" dateFormat="yy-mm-dd" (onSelect)="onDateRangeChange()" placeholder="Select date range" />
                                    </div>
                                    <div class="filter-item">
                                        <label>Status:</label>
                                        <p-select [(ngModel)]="statusFilter" [options]="statusOptions" optionLabel="label" optionValue="value" (onChange)="onStatusFilterChange()" placeholder="Select status" />
                                    </div>
                                    <div class="filter-actions">
                                        <p-button label="Clear Filters" icon="pi pi-times" (onClick)="clearFilters()" severity="secondary" />
                                    </div>
                                </div>
                            </div>

                            <div class="actions-bar">
                                <p-button label="Export to Excel" icon="pi pi-file-excel" (onClick)="exportBorrowingHistoryToExcel()" severity="success" />
                                <p-button label="Print Report" icon="pi pi-print" (onClick)="printBorrowingHistory()" />
                            </div>

                            <p-table [value]="borrowingHistory" [loading]="loading" [paginator]="true" [rows]="10" [rowsPerPageOptions]="[10, 25, 50]" [scrollable]="true" scrollHeight="500px">
                                <ng-template #header>
                                    <tr>
                                        <th>Borrower</th>
                                        <th>Type</th>
                    ...
                    <p-tabpanel value="1">
                        <p-card>
                            <div class="actions-bar">
                                <p-button label="Export to Excel" icon="pi pi-file-excel" (onClick)="exportBookPopularityToExcel()" severity="success" />
                                <p-button label="Print Report" icon="pi pi-print" (onClick)="printBookPopularity()" />
                            </div>

                            <div class="chart-container" *ngIf="borrowingTrendChart">
                                <h3>Top 10 Most Borrowed Books</h3>
                                <p-chart type="bar" [data]="borrowingTrendChart" [options]="{ responsive: true, maintainAspectRatio: false }" height="300px" />
                            </div>

                            <p-table [value]="bookPopularity" [loading]="loading" [paginator]="true" [rows]="10" [rowsPerPageOptions]="[10, 25, 50]">
                                <ng-template #header>
                                    <tr>
                                        <th>Rank</th>
                                        <th>Book Title</th>
                    ...
                    <p-tabpanel value="2">
                        <p-card>
                            <div class="actions-bar">
                                <p-button label="Export to Excel" icon="pi pi-file-excel" (onClick)="exportStudentPerformanceToExcel()" severity="success" />
                                <p-button label="Print Report" icon="pi pi-print" (onClick)="printStudentPerformance()" />
                            </div>

                            <div class="chart-container" *ngIf="performanceChart">
                                <h3>Student Performance Distribution</h3>
                                <p-chart type="pie" [data]="performanceChart" [options]="{ responsive: true, maintainAspectRatio: false }" height="300px" />
                            </div>

                            <p-table [value]="studentPerformance" [loading]="loading" [paginator]="true" [rows]="10" [rowsPerPageOptions]="[10, 25, 50]">
                                <ng-template #header>
                                    <tr>
                                        <th>Student Name</th>
                                        <th>LRN</th>
                                ...
                                <ng-template #body let-item>
                                    <tr>
                                        ...
                                        <td>
                                            <strong>{{ item.performanceScore }}</strong>
                                        </td>
                    ...
                    <p-tabpanel value="3">
                        <p-card>
                            <div class="filters-section">
                                ...
                                <div class="filter-grid">
                                    <div class="filter-item">
                                        ...
                                        <p-select [(ngModel)]="borrowerTypeFilter" [options]="borrowerTypeOptions" optionLabel="label" optionValue="value" (onChange)="onBorrowerTypeFilterChange()" placeholder="Select type" />
                                    </div>
                                    <div class="filter-item">
                                        <label>Status:</label>
                                        <p-select [(ngModel)]="statusFilter" [options]="statusOptions" optionLabel="label" optionValue="value" (onChange)="onStatusFilterChange()" placeholder="Select status" />
                            ...
                            <div class="actions-bar">
                                <p-button label="Export to Excel" icon="pi pi-file-excel" (onClick)="exportBorrowerListToExcel()" severity="success" />
                                <p-button label="Print Report" icon="pi pi-print" (onClick)="printBorrowerList()" />
                            </div>

                            <p-table [value]="borrowerList" [loading]="loading" [paginator]="true" [rows]="10" [rowsPerPageOptions]="[10, 25, 50]">
                                <ng-template #header>
                                    <tr>
                                        <th>Name</th>
                                        <th>Type</th>
...
export class ReportsComponent implements OnInit {
    ...
    async loadAllReports() {
        this.loading = true;
        try {
            await Promise.all([this.loadBorrowingHistory(), this.loadBookPopularity(), this.loadStudentPerformance(), this.loadBorrowerList()]);
        } catch (error) {
    ...
    async loadBorrowingHistory() {
        try {
            const startDate = this.dateRange[0] ? this.dateRange[0].toISOString().split('T')[0] : undefined;
            const endDate = this.dateRange[1] ? this.dateRange[1].toISOString().split('T')[0] : undefined;

            this.borrowingHistory = await this.reportsService.getBorrowingHistoryReport(startDate, endDate, this.statusFilter);
        } catch (error) {
    ...
    async loadBorrowerList() {
        try {
            this.borrowerList = await this.reportsService.getBorrowerListReport(this.borrowerTypeFilter, this.statusFilter);
        } catch (error) {
    ...
    exportBorrowingHistoryToExcel() {
        this.reportsService.exportToExcel(this.borrowingHistory, `borrowing-history-${new Date().toISOString().split('T')[0]}`, 'Borrowing History');
    }

    exportBookPopularityToExcel() {
        this.reportsService.exportToExcel(this.bookPopularity, `book-popularity-${new Date().toISOString().split('T')[0]}`, 'Book Popularity');
    }

    exportStudentPerformanceToExcel() {
        this.reportsService.exportToExcel(this.studentPerformance, `student-performance-${new Date().toISOString().split('T')[0]}`, 'Student Performance');
    }

    exportBorrowerListToExcel() {
        this.reportsService.exportToExcel(this.borrowerList, `borrower-list-${new Date().toISOString().split('T')[0]}`, 'Borrower List');
    }
    ...
    printBookPopularity() {
        const tableHtml = this.generateTableHtml(this.bookPopularity, ['rank', 'bookTitle', 'author', 'totalBorrows', 'currentlyBorrowed'], ['Rank', 'Book Title', 'Author', 'Total Borrows', 'Currently Borrowed']);
